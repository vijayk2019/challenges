	...
	public static void main(String[] args) {
		...
		//file logic goes here
				...
				while((line = bufferedReader.readLine()) != null){
					...
					String [] twoD_ip = firstdiv[0].split("\\s+"); // Seperating hole inputs in 2 strings
					...
					for(int i = 0; i < bricksdiv.length; i++){
						...
						int bric_z = Integer.parseInt(lastdiv[1].substring(lastdiv[1].lastIndexOf(",")+1, lastdiv[1].length() - 1));
						...
						int bric_z2 = Integer.parseInt(lastdiv[2].substring(lastdiv[2].lastIndexOf(",")+1, (lastdiv[2].length()-2)));
						bricks brickObj = new bricks(bric_x, bric_y, bric_z, bric_x2, bric_y2, bric_z2);
						if(brickObj.length <= hole1.length && brickObj.height <= hole1.height){
							if(result == null){
								result = lastdiv[0].substring(1, lastdiv[0].length());
							}
							else{
								result += ","+index;
							}
